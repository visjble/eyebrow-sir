...
BLOCKED_DOMAINS = [
    ...
    "analytics.google.com", "adtech.com", "optimizely.com",
    "clicktale.net", "mixpanel.com", "quantserve.com",
    "segment.io", "adjust.com", "amplitude.com"
]

# Add regex pattern matching for more sophisticated blocking
BLOCKED_PATTERNS = [
    r'\/ads?[\/\.]', r'\/analytics', r'\/pixel', r'\/tracker',
    r'\/beacon', r'\/tag(manager)?', r'\/stats', r'\/telemetry'
]
...
class RequestInterceptor(QWebEngineUrlRequestInterceptor):
    ...
    def interceptRequest(self, info):
        ...
        # Try to optimize resource loading if supported by PyQt version
        try:
            # Deprioritize non-essential resources
            if resource_type in [5, 7]:  # StyleSheet and Font resources
                info.setPriority(0)  # Low priority
            
            # Prioritize main document resources
            if resource_type == 0:  # MainFrame resource
                info.setPriority(2)  # High priority
        except:
            pass
...
class BrowserTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        try:
            self.profile.setHttpCacheMaximumSize(10 * 1024 * 1024)  # 10MB max
        except AttributeError:
            pass  # Method not available in this PyQt version
        ...
        # Try to preload DNS entries for common domains
        try:
            common_domains = ["google.com", "youtube.com", "github.com"]
            for domain in common_domains:
                self.page.resolveDnsEntry(domain)
        except AttributeError:
            pass  # Method not available in this PyQt version
...
class MinimalBrowser(QMainWindow):
    ...
    def reduce_memory_usage(self):
        ...
        # More aggressive tab hibernation for background tabs
        for i in range(self.tabs.count()):
            if i != self.tabs.currentIndex():
                tab = self.tabs.widget(i)
                if not tab.hibernated:
                    tab.hibernate()
...
def main():
    ...
    os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] += " --process-per-site --js-flags='--lite-mode' --enable-quic --enable-fast-unload --dns-prefetch-disable=false"
